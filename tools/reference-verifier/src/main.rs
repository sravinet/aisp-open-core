        ...
        SatResult::Sat => {
            ...
            if let Some(model) = solver.get_model() {
                println!("   📊 Example: unique={}, total={}, ambiguity={}", 
                        model.eval(&unique_parses, true).unwrap(),
                        model.eval(&total_parses, true).unwrap(),
                        model.eval(&ambiguity, true).unwrap());
            }
        }
        SatResult::Unsat => {
...
fn test_pipeline_improvement_verification(ctx: &Context) {
    ...
    match solver.check() {
        SatResult::Sat => {
            ...
            if let Some(model) = solver.get_model() {
                println!("   📊 Improvement factor: {}×", model.eval(&improvement, true).unwrap());
            }
        }
        SatResult::Unsat => {
...
fn test_single_step_pipeline(ctx: &Context) {
    ...
    match solver.check() {
        SatResult::Sat => {
            ...
            if let Some(model) = solver.get_model() {
                println!("         📊 Single-step improvement: {}×", model.eval(&improvement, true).unwrap());
            }
        },
